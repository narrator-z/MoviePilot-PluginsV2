...
# MoviePilot 核心 is_anime() 用正则把「剧名【年份】【地区/类型】【字幕】」这类苹果 CMS
# 中文命名误判为动漫（命中 【数字】\s*【 模式）。这里把片名里的 【...】/[...] 方括号
# 标记统一改写成 (...)，既保留年份/类型信息，又避开 is_anime 的括号误判，让电视剧正确归入 TV 库。
_BRACKET_TAG_RE = re.compile(r"[【\[]([^】\]]*)[】\]]")


def _normalize_cms_title(raw_title: str) -> str:
    """把 CMS 片名里的 【...】/[...] 标记改写为 (...)，避免被 MoviePilot 误判为动漫。"""
    if not raw_title:
        return raw_title
    return _BRACKET_TAG_RE.sub(r"(\1)", raw_title)
...
def _result_from_item(source: CmsSource, item: Mapping[str, Any]) -> CmsResult:
    title = _normalize_cms_title(_text(item.get("vod_name") or item.get("vod_en")))
